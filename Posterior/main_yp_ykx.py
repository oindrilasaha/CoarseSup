...
class ImageLabelDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_new = im[max(0,xmin-20):min(xmax+20,im.shape[0]),max(0,ymin-20):min(ymax+20, im.shape[1])]
        mask_new = mask[max(0,xmin-20):min(xmax+20,im.shape[0]),max(0,ymin-20):min(ymax+20, im.shape[1])]
        lbl_new = lbl[max(0,xmin-20):min(xmax+20,im.shape[0]),max(0,ymin-20):min(ymax+20, im.shape[1])]
        keyps_new = keyps[:,max(0,xmin-20):min(xmax+20,im.shape[0]),max(0,ymin-20):min(ymax+20, im.shape[1])]
...
def main():
    ...
    model_yp_ykymx = fcn_shallow(input_dim=15, nparts=11)
...
def train(train_loader_pascub,
    ...
    for i, (input_pascub, part, mask, keyps, _) in enumerate(train_loader_pascub):
        ...
        yp_out = model_yp_ykymx(torch.cat([input_pascub_var, keyps_var], 1))
...
def save_checkpoint(state, is_best, filename='*_checkpoint.pth.tar'):
    ...
    save_dir = os.path.join('./checkpoints_', work_dir) + "_ykx"
    cine(save_dir)
