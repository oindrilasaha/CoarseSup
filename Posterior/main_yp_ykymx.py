...
def main():
    ...
    model_yp_ykymx = fcn_shallow(input_dim=16, nparts=11)
...
def train(train_loader_pascub,
    ...
    for i, (input_pascub, part, mask, keyps, _) in enumerate(train_loader_pascub):
        ...
        yp_out = model_yp_ykymx(torch.cat([input_pascub_var, keyps_var, mask_var.unsqueeze(1)], 1))        
...
def save_checkpoint(state, is_best, filename='*_checkpoint.pth.tar'):
    ...
    save_dir = os.path.join('./checkpoints_', work_dir) + "_ykymx"
    cine(save_dir)
