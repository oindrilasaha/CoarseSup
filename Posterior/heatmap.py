...
class Heatmap(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.images_root,path))
        ...
        figure_ground[np.where(figure_ground<204)]=0
        figure_ground[np.where(figure_ground>0)]=1
        ...
        if self.transform is not None:
            (img, figure_ground, masks) = self.transform((img, figure_ground, masks.transpose(1,2,0)))
